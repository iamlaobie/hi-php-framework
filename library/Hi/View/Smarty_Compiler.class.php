 ...
 * @package Smarty
 */
class Smarty_Compiler extends Hi_View_Smarty
{
    // internal vars
    /**#@+
     * @access private
    ...
    function Smarty_Compiler ()
    {
        ...
        $this->_qstr_regexp = '(?:' . $this->_db_qstr_regexp . '|' .
         $this->_si_qstr_regexp . ')';
        ...
        $this->_obj_restricted_param_regexp = '(?:' . '(?:' . $this->_var_regexp .
         '|' . $this->_num_const_regexp . ')(?:' . $this->_obj_ext_regexp .
         '(?:\((?:(?:' . $this->_var_regexp . '|' . $this->_num_const_regexp .
        ...
        $this->_obj_call_regexp = '(?:' . $this->_obj_start_regexp . '(?:' .
         $this->_obj_params_regexp . ')?(?:' . $this->_dvar_math_regexp . '(?:' .
        ...
        $this->_mod_regexp = '(?:\|@?\w+(?::(?:\w+|' .
         $this->_num_const_regexp . '|' . $this->_obj_call_regexp . '|' .
         $this->_avar_regexp . '|' . $this->_qstr_regexp . '))*)';
        ...
        $this->_param_regexp = '(?:\s*(?:' .
         ...
         $this->_num_const_regexp . '|\w+)(?>' . $this->_mod_regexp . '*)\s*)';
        ...
        $this->_parenth_param_regexp = '(?:\((?:\w+|' .
         $this->_param_regexp . '(?:\s*,\s*(?:(?:\w+|' . $this->_param_regexp .
         ')))*)?\))';
        ...
        $this->_func_call_regexp = '(?:' .
         $this->_func_regexp . '\s*(?:' . $this->_parenth_param_regexp . '))';
    }
    /**
     * compile a resource
     *
    ...
    function _compile_file ($resource_name, $source_content, &$compiled_content)
    {
        ...
        if (count($this->_plugins['prefilter']) > 0) {
            foreach ($this->_plugins['prefilter'] as $filter_name => $prefilter) {
                ...
                if ($prefilter[3] || is_callable($prefilter[0])) {
                    ...
                    "[plugin] prefilter '$filter_name' is not implemented");
                }
            }
        }
                ...
                for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp <
                 $for_max2; $curr_sp ++) {
                    if ($this->php_handling == SMARTY_PHP_PASSTHRU) {
                        /* echo php contents */
                        $text_blocks[$curr_tb] = str_replace(
                        '%%%SMARTYSP' . $curr_sp . '%%%', 
                        '<?php echo \'' .
                        ...
                        if ($this->php_handling == SMARTY_PHP_QUOTE) {
                            ...
                            if ($this->php_handling == SMARTY_PHP_REMOVE) {
                                ...
                                $sp_match[1][$curr_sp] = preg_replace(
                                '~(<\?(?!php|=|$))~i', 
                                '<?php echo \'\\1\'?>' . "\n", 
                                $sp_match[1][$curr_sp]);
                                $text_blocks[$curr_tb] = str_replace(
        ...
        if (count($this->_tag_stack) > 0) {
            ...
            $this->_syntax_error(
            "unclosed tag \{$_open_tag} (opened line $_line_no).", E_USER_ERROR, 
            __FILE__, __LINE__);
            return;
        }
        ...
        for ($i = 0, $for_max = count($compiled_tags); $i < $for_max; $i ++) {
            ...
            $text_blocks[$i] = str_replace('<?', $tag_guard, 
            $text_blocks[$i]);
            $compiled_tags[$i] = str_replace('<?', $tag_guard, 
            $compiled_tags[$i]);
        ...
        if (! empty($this->_cache_serial)) {
            $compiled_content = "<?php \$this->_cache_serials['" .
             $this->_cache_include . "'] = '" . $this->_cache_serial . "'; ?>" .
             $compiled_content;
        }
    ...
    function _compile_tag ($template_tag)
    {
        ...
        if (! preg_match(
        '~^(?:(' . $this->_num_const_regexp . '|' . $this->_obj_call_regexp . '|' .
         $this->_var_regexp . '|\/?' . $this->_reg_obj_regexp . '|\/?' .
        ...
        if (preg_match('~^\/?' . $this->_reg_obj_regexp . '$~', $tag_command)) {
            return $this->_compile_registered_object_tag($tag_command, 
        ...
        switch ($tag_command) {
            ...
            case 'else':
                ...
                if ($_open_tag != 'if' && $_open_tag != 'elseif')
                    $this->_syntax_error('unexpected {else}', E_USER_ERROR, 
                    __FILE__, __LINE__);
                else
                    $this->_push_tag('else');
                return '<?php else: ?>';
            case 'elseif':
                list ($_open_tag) = end($this->_tag_stack);
                if ($_open_tag != 'if' && $_open_tag != 'elseif')
                    $this->_syntax_error('unexpected {elseif}', E_USER_ERROR, 
                    __FILE__, __LINE__);
                if ($_open_tag == 'if')
            ...
            default:
                ...
                $output)) {
                    ...
                    if ($this->_compile_block_tag($tag_command, $tag_args, 
                    $tag_modifier, $output)) {
                        return $output;
                    } else 
                        if ($this->_compile_custom_tag($tag_command, $tag_args, 
                        $tag_modifier, $output)) {
                            return $output;
                        } else {
                            $this->_syntax_error(
                            "unrecognized tag '$tag_command'", E_USER_ERROR, 
                            __FILE__, __LINE__);
                        }
        }
    }
    ...
    function _compile_compiler_tag ($tag_command, $tag_args, &$output)
    {
         ...
         */
            ...
            if ($plugin_file = $this->_get_plugin_filepath('compiler', 
            $tag_command)) {
                $found = true;
                ...
                if (! is_callable($plugin_func)) {
                    ...
                    $plugin_func, null, null, null, true);
                }
            }
        /*
        ...
        if ($found) {
            if ($have_function) {
                $output = call_user_func_array($plugin_func, 
                array($tag_args, &$this));
                if ($output != '') {
                    $output = '<?php ' .
     ...
     * @return boolean
     */
    function _compile_block_tag ($tag_command, $tag_args, $tag_modifier, 
    &$output)
    {
        if (substr($tag_command, 0, 1) == '/') {
            $start_tag = false;
         ...
         */
            ...
            if ($plugin_file = $this->_get_plugin_filepath('block', 
            $tag_command)) {
                $found = true;
                include_once $plugin_file;
        ...
        if ($start_tag) {
            ...
            $_cache_attrs);
            $output .= "$_cache_attrs\$this->_tag_stack[] = array('$tag_command', array(" .
             implode(',', $arg_list) . ')); ';
    ...
    &$output)
    {
         ...
         */
            ...
            if ($plugin_file = $this->_get_plugin_filepath('function', 
            $tag_command)) {
                $found = true;
                include_once $plugin_file;
        ...
        $_cache_attrs);
        $output = $this->_compile_plugin_call('function', $tag_command) .
         '(array(' . implode(',', $arg_list) . "), \$this)";
        if ($tag_modifier != '') {
            $this->_parse_modifiers($output, $tag_modifier);
        }
        if ($output != '') {
            $output = '<?php ' . $_cacheable_state . $_cache_attrs . 'echo ' .
             $output . ';' .
    ...
    function _compile_registered_object_tag ($tag_command, $attrs, $tag_modifier)
    {
        ...
        if ($this->_reg_objects[$object][2]) {
            ...
            $args = "array(" . implode(',', (array) $arg_list) .
             "), \$this";
        } else {
        ...
        if (! is_object($this->_reg_objects[$object][0])) {
            ...
            $this->_current_file, $this->_current_line_no, __FILE__, __LINE__);
    ...
    function _compile_insert_tag ($tag_args)
    {
        ...
        if (empty($name)) {
            return $this->_syntax_error("missing insert name", E_USER_ERROR, 
            __FILE__, __LINE__);
        }
        if (! preg_match('~^\w+$~', $name)) {
            return $this->_syntax_error(
            "'insert: 'name' must be an insert function name", E_USER_ERROR, 
            __FILE__, __LINE__);
        }
    ...
    function _compile_include_php_tag ($tag_args)
    {
        ...
        $assign_var = (empty($attrs['assign'])) ? '' : $this->_dequote(
        $attrs['assign']);
        $once_var = (empty($attrs['once']) || $attrs['once'] == 'false') ? 'false' : 'true';
        $arg_list = array();
        foreach ($attrs as $arg_name => $arg_value) {
            if ($arg_name != 'file' and $arg_name != 'once' and
             $arg_name != 'assign') {
                if (is_bool($arg_value))
                    $arg_value = $arg_value ? 'true' : 'false';
                $arg_list[] = "'$arg_name' => $arg_value";
        ...
        $_params = "array('smarty_file' => " . $attrs['file'] .
         ", 'smarty_assign' => '$assign_var', 'smarty_once' => $once_var, 'smarty_include_vars' => array(" .
         implode(',', $arg_list) . "))";
    ...
    function _compile_section_start ($tag_args)
    {
        ...
        foreach ($attrs as $attr_name => $attr_value) {
            switch ($attr_name) {
                ...
                default:
                    $this->_syntax_error(
                    "unknown section attribute - '$attr_name'", E_USER_ERROR, 
                    __FILE__, __LINE__);
                    break;
            }
        }
        ...
        if (! isset($attrs['start']) && ! isset($attrs['step']) &&
         ! isset($attrs['max'])) {
    ...
    function _compile_foreach_start ($tag_args)
    {
        ...
        if (! preg_match('~^\w+$~', $item)) {
            return $this->_syntax_error(
            "'foreach: 'item' must be a variable name (literal string)", 
            E_USER_ERROR, __FILE__, __LINE__);
        }
        if (isset($attrs['key'])) {
            ...
            if (! preg_match('~^\w+$~', $key)) {
                return $this->_syntax_error(
                "foreach: 'key' must to be a variable name (literal string)", 
                E_USER_ERROR, __FILE__, __LINE__);
            }
            ...
            switch (strtolower($token)) {
                ...
                case 'is':
                    ...
                    if ($tokens[$i - 1] == ')')
                       ...
                       what to operate on. */
                    $is_arg = implode(' ', 
                    array_slice($tokens, $is_arg_start, $i - $is_arg_start));
                    /* Pass all tokens from next one until the end to the
                       'is' expression parsing function. The function will
                       return modified tokens, where the first one is the result
                       of the 'is' expression and the rest are the tokens it
                       didn't touch. */
                    $new_tokens = $this->_parse_is_expr($is_arg, 
                ...
                default:
                    if (preg_match('~^' . $this->_func_regexp . '$~', $token)) {
                        ...
                        $this->security_settings['IF_FUNCS'])) {
                            $this->_syntax_error(
                            "(secure mode) '$token' not allowed in if statement", 
                            E_USER_ERROR, __FILE__, __LINE__);
                        }
                    } elseif (preg_match('~^' . $this->_var_regexp . '$~', 
                    $token) && (strpos('+-*/^%&|', substr($token, - 1)) === false) &&
    ...
    {
        $arg_list = array();
        if (isset($type) && isset($name) && isset($this->_plugins[$type]) &&
        ...
        switch ($expr_type) {
            case 'even':
                if (isset($tokens[$expr_end]) && $tokens[$expr_end] == 'by') {
                    ...
                    $expr = "!(1 & ($is_arg / " .
                     $this->_parse_var_props($expr_arg) . "))";
                } else
                    $expr = "!(1 & $is_arg)";
                break;
            case 'odd':
    ...
    function _parse_attrs ($tag_args)
    {
        ...
        preg_match_all(
        '~(?:' . $this->_obj_call_regexp . '|' . $this->_qstr_regexp . ' | (?>[^"\'=\s]+)
                         )+ |
                         [=]
                        ~x', $tag_args, $match);
        $tokens = $match[0];
        ...
        if ($state != 0) {
            if ($state == 1) {
                $this->_syntax_error(
                "expecting '=' after attribute name '$last_token'", E_USER_ERROR, 
                __FILE__, __LINE__);
            } else {
    ...
    function _parse_var_props ($val)
    {
        $val = trim($val);
        if (preg_match(
        '~^(' . $this->_obj_call_regexp . '|' . $this->_dvar_regexp . ')(' .
         $this->_mod_regexp . '*)$~', $val, $match)) {
            ...
            if (! empty($this->default_modifiers) &&
             ! preg_match('~(^|\|)smarty:nodefaults($|\|)~', $modifiers)) {
                $_default_mod_string = implode('|', 
                (array) $this->default_modifiers);
                $modifiers = empty($modifiers) ? $_default_mod_string : $_default_mod_string .
                 '|' . $modifiers;
            }
            $this->_parse_modifiers($return, $modifiers);
            return $return;
        } elseif (preg_match(
        '~^' . $this->_db_qstr_regexp . '(?:' . $this->_mod_regexp . '*)$~', 
        $val)) {
            // double quoted text
            preg_match(
            '~^(' . $this->_db_qstr_regexp . ')(' . $this->_mod_regexp . '*)$~', 
        ...
        '~^' . $this->_num_const_regexp . '(?:' . $this->_mod_regexp . '*)$~', 
        $val)) {
            // numerical constant
            preg_match(
            '~^(' . $this->_num_const_regexp . ')(' . $this->_mod_regexp . '*)$~', 
        ...
        '~^' . $this->_si_qstr_regexp . '(?:' . $this->_mod_regexp . '*)$~', 
        $val)) {
            // single quoted text
            preg_match(
            '~^(' . $this->_si_qstr_regexp . ')(' . $this->_mod_regexp . '*)$~', 
        ...
        '~^' . $this->_cvar_regexp . '(?:' . $this->_mod_regexp . '*)$~', $val)) {
            // config var
            return $this->_parse_conf_var($val);
        } elseif (preg_match(
        '~^' . $this->_svar_regexp . '(?:' . $this->_mod_regexp . '*)$~', $val)) {
            // section var
    ...
    function _expand_quoted_text ($var_expr)
    {
        ...
        if (preg_match_all(
        '~(?:\`(?<!\\\\)\$' . $this->_dvar_guts_regexp . '(?:' .
         $this->_obj_ext_regexp . ')*\`)|(?:(?<!\\\\)\$\w+(\[[a-zA-Z0-9]+\])*)~', 
        $var_expr, $_match)) {
            $_match = $_match[0];
            $_replace = array();
            foreach ($_match as $_var) {
                $_replace[$_var] = '".(' .
                 $this->_parse_var(str_replace('`', '', $_var)) . ')."';
            }
    ...
    {
        $_has_math = false;
        $_math_vars = preg_split(
        '~(' . $this->_dvar_math_regexp . '|' . $this->_qstr_regexp . ')~', 
            ...
            foreach ($_math_vars as $_k => $_math_var) {
                ...
                if (! empty($_math_var) || is_numeric($_math_var)) {
                    ...
                    '~^' . $this->_dvar_math_regexp . '$~', $_math_var)) {
                        $_has_math = true;
                        if (! empty($_complete_var) || is_numeric(
                        $_complete_var)) {
        ...
        if (! $_has_math) {
            ...
            '~(?:^\w+)|' . $this->_obj_params_regexp . '|(?:' .
             $this->_var_bracket_regexp . ')|->\$?\w+|\.\$?\w+|\S+~', $_var_ref, 
            $match);
            $_indexes = $match[0];
                        ...
                        if (substr($_index, 0, 2) == '->') {
                            if (substr($_index, 2, 2) == '__') {
                             ...
                             substr($_index, 2, 1) == '_') {
                                ...
                                if ($this->security) {
                                    $this->_syntax_error(
                                    '(secure) call to dynamic object member is not allowed', 
                                    E_USER_ERROR, __FILE__, __LINE__);
                                } else {
                                    $_output .= '->{(($_var=$this->_tpl_vars[\'' .
                                     substr($_index, 3) .
    ...
    function _add_plugin ($type, $name, $delayed_loading = null)
    {
        ...
        if (! isset($this->_plugin_info[$type][$name])) {
            ...
            $this->_current_line_no, $delayed_loading);
        }
    }
    /**
    ...
    function _compile_smarty_ref (&$indexes)
    {
        ...
        foreach ($indexes as $_index_no => $_index) {
            if (substr($_index, 0, 1) != '.' && $_index_no < 2 ||
             ! preg_match('~^(\.|\[|->)~', $_index)) {
                $this->_syntax_error(
                '$smarty' . implode('', array_slice($indexes, 0, 2)) .
        ...
        switch ($_ref) {
            ...
            case 'const':
                if ($this->security &&
                 ! $this->security_settings['ALLOW_CONSTANTS']) {
                    $this->_syntax_error(
                    "(secure mode) constants not permitted", E_USER_WARNING, 
                    __FILE__, __LINE__);
                    return;
                }
                array_shift($indexes);
            ...
            default:
                $this->_syntax_error(
                '$smarty.' . $_ref . ' is an unknown reference', E_USER_ERROR, 
                __FILE__, __LINE__);
                break;
        }
        if (isset($_max_index) && count($indexes) > $_max_index) {
            $this->_syntax_error(
            '$smarty' . implode('', $indexes) . ' is an invalid reference', 
            E_USER_ERROR, __FILE__, __LINE__);
        }
        array_shift($indexes);
        return $compiled_ref;
    ...
    function _compile_plugin_call ($type, $name)
    {
        if (isset($this->_plugins[$type][$name])) {
            ...
            if (is_array($this->_plugins[$type][$name][0])) {
                return ((is_object($this->_plugins[$type][$name][0][0])) ? "\$this->_plugins['$type']['$name'][0][0]->"    /* method callback */
        ...
        if (count($this->_plugins['prefilter']) > 0) {
            foreach ($this->_plugins['prefilter'] as $filter_name => $prefilter) {
                if ($prefilter === false) {
                    ...
                    $_params = array(
                    'plugins' => array(
                    array('prefilter', $filter_name, null, null, false)));
        ...
        if (count($this->_plugins['postfilter']) > 0) {
            foreach ($this->_plugins['postfilter'] as $filter_name => $postfilter) {
                if ($postfilter === false) {
                    ...
                    $_params = array(
                    'plugins' => array(
                    array('postfilter', $filter_name, null, null, false)));
                    require_once (SMARTY_CORE_DIR . 'core.load_plugins.php');
                    smarty_core_load_plugins($_params, $this);
                }
            }
    ...
    {
        $this->_trigger_fatal_error("syntax error: $error_msg", 
    ...
    function _push_cacheable_state ($type, $name)
    {
        ...
        $_ret = 'if ($this->caching && !$this->_cache_including): echo \'{nocache:' .
         $this->_cache_serial . '#' . $this->_nocache_count . '}\'; endif;';
        return $_ret;
    }
    /**
    ...
    function _pop_tag ($close_tag)
    {
        ...
        if (count($this->_tag_stack) > 0) {
            ...
            if ($close_tag == 'if' &&
             ($_open_tag == 'else' || $_open_tag == 'elseif')) {
